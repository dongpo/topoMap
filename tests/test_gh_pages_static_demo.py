...
OUTPUT = ROOT / "artifacts/tmp/gh-pages"
...
class StaticPagesDemoTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        subprocess.run(
            [sys.executable, "scripts/build_gh_pages.py", "--output", "artifacts/tmp/gh-pages"],
            cwd=ROOT,
            check=True,
        )
        cls.index = (OUTPUT / "index.html").read_text(encoding="utf-8")
        cls.app = (OUTPUT / "app.js").read_text(encoding="utf-8")
        cls.payload = json.loads((OUTPUT / "data/scenarios.json").read_text(encoding="utf-8"))
    ...
    def test_school_is_accepted_fifteen_point_public_safe_replay(self) -> None:
        ...
        self.assertEqual(
            school["map"]["coordinate_policy"], "normalized-public-replay-not-source-geography"
        )
        self.assertTrue(
            all(
    ...
    def test_road_preserves_accepted_identity_counts_and_label(self) -> None:
        ...
        self.assertEqual([len(item["geometry"]["coordinates"]) for item in features], [4, 3, 4])
        self.assertEqual([item["properties"]["ROADNAME"] for item in features], ["中山街"] * 3)
        self.assertIn("K14_ROAD", road["title"])
        self.assertIn("line-following 中山街", road["subtitle"])
        self.assertEqual(
            road["map"]["coordinate_policy"], "normalized-public-replay-not-source-geography"
        )
    ...
    def test_build_is_normalized_hatched_replay_with_activation_disabled(self) -> None:
        ...
        self.assertEqual(build["map"]["geojson"]["features"][0]["geometry"]["type"], "Polygon")
    ...
    def test_release_excludes_private_archives_credentials_and_live_api(self) -> None:
        ...
        for secret_marker in ("OPENAI_API_KEY", "NEO4J_PASSWORD", "sk-proj-", "/api/v1/runs"):
            self.assertNotIn(secret_marker, corpus)

    def test_maplibre_is_vendored_and_release_manifest_is_complete(self) -> None:
