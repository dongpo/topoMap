...
let runId=null,poller=null,map=null,resultMarker=null,currentMode=null,config=null;
...
function hydrantPreviewMarker(feature){
  const properties=feature?.properties||{},portrayal=properties.nma_portrayal||{},marker=document.createElement('div');
  marker.className='hydrant-preview-marker';marker.setAttribute('role','img');marker.setAttribute('aria-label',`Non-authoritative symbolic fire-hydrant preview for classification ${properties.nma_classification||'9350906'}`);
  marker.innerHTML=`<svg viewBox="0 0 48 56" role="img" aria-label="Symbolic fire-hydrant pictogram"><path class="hydrant-outline" d="M20 3h8v7h-8zM15 17c0-6 4-9 9-9s9 3 9 9v2H15zM14 18h20v25H14zM8 23h6v11H8zM34 23h6v11h-6zM5 25h5v7H5zM38 25h5v7h-5zM10 43h28v8H10z"></path><path class="hydrant-mark" d="M18 29h12M24 23v12"></path></svg><span>${esc(properties.nma_classification||'9350906')}</span>`;
  marker.title=`Symbolic preview only · Point · line code ${portrayal.line_code||'—'} · color code ${portrayal.color_code||'—'} · authoritative render: ${String(properties.authoritative_render)}`;
  return marker;
}
async function renderMap(mode){
  ...
  map.addSource('source',{type:'geojson',data:source});map.addLayer({id:'source',type:'circle',source:'source',paint:{'circle-radius':12,'circle-color':'#98a49f','circle-stroke-color':'#fff','circle-stroke-width':2}});map.addSource('result',{type:'geojson',data:result});map.addLayer({id:'result',type:'circle',source:'result',paint:{'circle-radius':27,'circle-color':mode==='REPLAY'?'#8a4dcc':'#1565c0','circle-opacity':.18,'circle-stroke-color':mode==='REPLAY'?'#8a4dcc':'#1565c0','circle-stroke-width':3}});const feature=result.features[0],coords=feature.geometry.coordinates,properties=feature.properties||{},portrayal=properties.nma_portrayal||{};resultMarker=new maplibregl.Marker({element:hydrantPreviewMarker(feature),anchor:'center'}).setLngLat(coords).addTo(map);resultMarker.getElement().setAttribute('aria-label',`Non-authoritative symbolic fire-hydrant preview for classification ${properties.nma_classification||'9350906'}`);$('map-symbol-notice').innerHTML=`<strong>NON-AUTHORITATIVE SYMBOLIC PREVIEW</strong><span>Hydrant pictogram for derived Point ${esc(properties.nma_classification||'9350906')} · line code ${esc(portrayal.line_code||'—')} · color code ${esc(portrayal.color_code||'—')} (${esc(portrayal.observed_color||'unresolved')}). Official glyph and physical profile remain gated.</span>`;map.flyTo({center:coords,zoom:15});pill($('map-status'),mode==='LIVE'?'LIVE RESULT':'REPLAY RESULT');
}
