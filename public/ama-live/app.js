...
let runId=null,poller=null,map=null,resultMarker=null,currentMode=null,config=null,featureSearchTimer=null;
...
function renderActionGraph(data){
  const host=$('action-graph'),all=data?.nodes||[],evidence=all.filter(node=>node.type==='RetrievedEvidence').slice(0,8),nodes=[...all.filter(node=>node.type!=='RetrievedEvidence'),...evidence],edges=data?.edges||[];
...
function renderTrust(record){
  ...
  const hashes=[proposal.proposal_hash,authorization.proposal_hash,provenance.executed_proposal_hash],denied=authorization.decision==='DENIED'&&execution.status==='NOT_RUN',identity=Boolean(hashes[0])&&new Set(hashes).size===1;
  $('proposal-hash').textContent=compact(hashes[0]);$('proposal-hash').title=hashes[0]||'';$('authorized-hash').textContent=compact(hashes[1]);$('authorized-hash').title=hashes[1]||'';$('executed-hash').textContent=denied?'NOT RUN':compact(hashes[2]);$('executed-hash').title=hashes[2]||'';pill($('identity-status'),denied?'FAIL-CLOSED PASS':identity?'HASH IDENTITY PASS':'HASH IDENTITY FAIL');
...
function featureSymbolMarker(feature,record){
  const properties=feature?.properties||{},resolved=record?.intent_resolution?.feature||{},code=resolved.code||properties.nma_classification||'9350906',name=resolved.name||'消防栓',asset=resolved.symbol_asset||'/symbols/fire-hydrant.svg',marker=document.createElement('div');
  marker.className='feature-symbol-marker';marker.setAttribute('role','img');marker.setAttribute('aria-label',`Specification-derived symbol preview for ${name} ${code}`);
  marker.innerHTML=`<img src="${esc(asset)}" alt="${esc(name)} specification-derived SVG preview"><span>${esc(code)}</span>`;
  marker.title=`Specification-derived implementation preview · ${name} · TerrainID ${code}`;
  return marker;
}
async function ensureMap(){
  if(map)return map;
  map=new maplibregl.Map({container:'map',center:[121.0236,24.7745],zoom:14,attributionControl:true,style:{version:8,sources:{nlsc:{type:'raster',tiles:['https://wmts.nlsc.gov.tw/wmts/EMAP/default/GoogleMapsCompatible/{z}/{y}/{x}'],tileSize:256,maxzoom:19,attribution:'© 內政部國土測繪中心'}},layers:[{id:'bg',type:'background',paint:{'background-color':'#e7ece8'}},{id:'nlsc-emap-basemap',type:'raster',source:'nlsc',paint:{'raster-opacity':.78}}]}});
  map.addControl(new maplibregl.NavigationControl({showCompass:false}),'top-right');await new Promise(resolve=>map.on('load',resolve));return map;
}
async function renderMap(mode,record){
  await ensureMap();if(resultMarker){resultMarker.remove();resultMarker=null}
  for(const id of ['source','result']){if(map.getLayer(id))map.removeLayer(id);if(map.getSource(id))map.removeSource(id)}
  const resolved=record?.intent_resolution?.feature||{},code=resolved.code||'—',name=resolved.name||'Unresolved feature';
  if(record?.status!=='PASS'&&mode!=='REPLAY'){
    const preview=resolved.symbol_asset?`<img src="${esc(resolved.symbol_asset)}" alt="${esc(name)} SVG preview">`:'';
    $('map-symbol-notice').innerHTML=`${preview}<strong>NO AUTHORIZED GIS OUTPUT</strong><span>${esc(name)} · TerrainID ${esc(code)} was accepted for GraphRAG retrieval, but GIS execution was denied because no proposal-bound feature fixture is authorized. The NLSC contextual map remains unchanged.</span>`;pill($('map-status'),'NO GIS OUTPUT');return;
  }
  const resultPath=mode==='REPLAY'?'/ama/demo/replay/result':`/ama/run/${runId}/result`,[source,result]=await Promise.all([api('/ama/source'),api(resultPath)]);
  map.addSource('source',{type:'geojson',data:source});map.addLayer({id:'source',type:'circle',source:'source',paint:{'circle-radius':12,'circle-color':'#6d7773','circle-stroke-color':'#fff','circle-stroke-width':2}});
  map.addSource('result',{type:'geojson',data:result});map.addLayer({id:'result',type:'circle',source:'result',paint:{'circle-radius':27,'circle-color':mode==='REPLAY'?'#8a4dcc':'#006f8b','circle-opacity':.18,'circle-stroke-color':mode==='REPLAY'?'#8a4dcc':'#006f8b','circle-stroke-width':3}});
  const feature=result.features[0],coords=feature.geometry.coordinates,properties=feature.properties||{},portrayal=properties.nma_portrayal||{};
  resultMarker=new maplibregl.Marker({element:featureSymbolMarker(feature,record),anchor:'center'}).setLngLat(coords).addTo(map);
  $('map-symbol-notice').innerHTML=`<img src="${esc(resolved.symbol_asset||'/symbols/fire-hydrant.svg')}" alt="${esc(name)} SVG preview"><strong>SPECIFICATION-DERIVED SVG PREVIEW</strong><span>${esc(name)} · TerrainID ${esc(code)} · line code ${esc(portrayal.line_code||'—')} · color code ${esc(portrayal.color_code||'—')}. The existing reviewed SVG is reused; independent cartographic sign-off and physical portrayal gates remain visible.</span>`;
  map.flyTo({center:coords,zoom:17});pill($('map-status'),mode==='LIVE'?'LIVE RESULT':'REPLAY RESULT');
}
async function renderRecord(record,mode){
  ...
  const resolution=record.intent_resolution||{},feature=resolution.feature||{};$('intent-forms').innerHTML=dl([['Resolution',resolution.status||'FROZEN REPLAY'],['TerrainID',feature.code],['Feature',feature.name],['Execution mode',record.execution_mode||'AUTHORIZED FIXTURE'],['Execution contract',record.execution_contract_intent||'Query-only / frozen replay']]);
  $('m-graphrag').textContent=ms(record.timing_ms?.graphrag);$('m-plan').textContent=ms(record.timing_ms?.llm_planning??record.timing_ms?.agent_planning);$('m-auth').textContent=ms(record.timing_ms?.authorization);$('m-gis').textContent=ms(record.timing_ms?.gis_execution);$('m-verify').textContent=ms(record.timing_ms?.verification);$('m-total').textContent=ms(record.timing_ms?.end_to_end);
...
async function begin(){
  try{$('error').hidden=true;$('run').disabled=true;$('tamper').disabled=true;const record=await api('/ama/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({intent:$('intent').value})});runId=record.run_id;setMode('LIVE',runId);renderStages(record);$('run-meta').textContent=runId;poller=setInterval(async()=>{try{const current=await api(`/ama/run/${runId}`);renderStages(current);renderTiming(current.timing_ms);if(['PASS','ABSTAINED'].includes(current.status)){clearInterval(poller);$('run').disabled=false;$('tamper').disabled=current.status!=='PASS';await renderRecord(current,'LIVE')}else if(current.status==='FAILED'){showFailure(new Error(current.failure?.message||'Live run failed.'))}}catch(error){showFailure(error)}},750)}catch(error){showFailure(error)}
}
...
async function reset(){
  try{clearInterval(poller);await api('/ama/reset',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});runId=null;currentMode=null;setMode(null);$('tamper').disabled=true;$('tamper-result').hidden=true;$('error').hidden=true;$('run').disabled=false;renderStages({status:'WAITING',stages:{}});for(const id of ['proposal-status','identity-status','map-status','verification-status','provenance-status'])pill($(id),'WAITING');$('run-meta').textContent='Graph-wide feature admission';$('retrieved-graph').className='graph empty';$('retrieved-graph').textContent='Run AMA or choose replay.';renderRelationLegend('retrieved-relations',[],'Select LIVE or VERIFIED REPLAY to load graph relations.');$('action-graph').className='graph trace-graph empty';$('action-graph').textContent='Run AMA or choose replay.';renderRelationLegend('action-relations',[],'Select LIVE or VERIFIED REPLAY to load action relations.');$('constraints').innerHTML='<tr><td colspan="5" class="muted">No selected run.</td></tr>';$('map-symbol-notice').innerHTML='<strong>MAP RESULT NOT LOADED</strong><span>Run AMA or choose verified replay.</span>';if(resultMarker){resultMarker.remove();resultMarker=null}if(map){for(const id of ['source','result']){if(map.getLayer(id))map.removeLayer(id);if(map.getSource(id))map.removeSource(id)}}}catch(error){showFailure(error)}
}
...
async function init(){
  const [loadedConfig,domain,rq1]=await Promise.all([api('/ama/config'),api('/ama/demo/domain-kg'),api('/ama/demo/rq1-comparison')]);config=loadedConfig;$('intent').value=config.canonical_intent;$('catalog-count').textContent=`${config.queryable_terrainid_count} exact seven-digit TerrainIDs indexed`;$('intent-forms').innerHTML=dl([['Admission','All indexed TerrainIDs'],['Live GIS fixture codes',config.live_execution_fixture_codes.join(', ')],['Safety boundary','Other features complete as evidence-bearing, no-mutation abstentions']]);$('deployment-footer').textContent=`AMA-DEMO-02 · ${config.deployment}`;renderGraph('domain-graph',domain);renderRelationLegend('retrieved-relations',[],'Select LIVE or VERIFIED REPLAY to load graph relations.');renderRelationLegend('action-relations',[],'Select LIVE or VERIFIED REPLAY to load action relations.');renderRQ1(rq1);renderStages({status:'WAITING',stages:{}});setMode(null);await Promise.all([searchFeatures(),loadResearchExamples()]);
}
function featureButton(item){return `<button type="button" class="feature-match" data-code="${esc(item.code)}"><b>${esc(item.code)}</b><span>${esc(item.name)}</span><small>${esc(item.execution_support.replaceAll('-',' '))}</small>${item.symbol_asset?`<img src="${esc(item.symbol_asset)}" alt="">`:''}</button>`}
async function loadResearchExamples(){
  const rows=await Promise.all((config.research_example_codes||[]).map(code=>api(`/ama/features?query=${code}&limit=1`)));
  $('research-examples').innerHTML=rows.flatMap(row=>row.matches).map(featureButton).join('');
}
async function searchFeatures(){
  const query=$('intent').value.trim();if(!query){$('feature-matches').innerHTML='';return}
  try{const result=await api(`/ama/features?query=${encodeURIComponent(query)}&limit=8`);$('feature-matches').innerHTML=result.matches.map(featureButton).join('')||'<span class="feature-none">No canonical TerrainID match. AMA will abstain safely.</span>'}catch(error){$('feature-matches').innerHTML=`<span class="feature-none">${esc(error.message)}</span>`}
}
function chooseFeature(event){const button=event.target.closest('[data-code]');if(!button)return;const code=button.dataset.code,name=button.querySelector('span')?.textContent||'feature';$('intent').value=`Create a safe derived mapping feature for ${name}, TerrainID ${code}, using reviewed graph knowledge; preserve unresolved constraints and leave the authoritative source unchanged.`;searchFeatures()}
$('feature-matches').addEventListener('click',chooseFeature);$('research-examples').addEventListener('click',chooseFeature);
$('intent').addEventListener('input',()=>{clearTimeout(featureSearchTimer);featureSearchTimer=setTimeout(searchFeatures,220)});
